    ...
    img.onload = function () {
      ...
      setTimeout(() => {
        ...
        function processImageChunk() {
          const endY = Math.min(y + 50, height);
        ...
        function processBoundaries() {
          ...
          function processBoundaryChunk() {
            ...
            if (y < height) {
              requestAnimationFrame(processBoundaryChunk);
            } else {
              processInterior();
            }
          }

          function processInterior() {
            let y = 1;

            function processInteriorChunk() {
              const endY = Math.min(y + 50, height - 1);
              ...
              if (y < height - 1) {
                requestAnimationFrame(processInteriorChunk);
              } else {
                processRelaxation();
              }
            }
          ...
          function processRelaxation() {
            ...
            function processRelaxationIteration() {
              const batchSize = 10;
              const endIter = Math.min(iter + batchSize, ITERATIONS);
              ...
              if (iter < ITERATIONS) {
                requestAnimationFrame(processRelaxationIteration);
              } else {
                finalizeImage();
              }
            }
